...
public class BuildDatabase {
	...
	private static void selectWithPreparedStatement() throws SQLException {
		...
		String CreateTable = "CREATE TABLE IF NOT EXISTS GWS_GAME(ID INT PRIMARY KEY, NAME VARCHAR(255), STATUS VARCHAR(255))";
		String InsertTable = "INSERT INTO GWS_GAME VALUES(1, 'Ted', 'Is Currently Awesome')";
		...
		if (rsm.next()) {
			...
			try {
				...
				while (rs.next()) {
					logger.info(rs.getInt("id") + " " + rs.getString("name") + " " + rs.getString("status"));
				}
